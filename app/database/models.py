...
class Medicine(Base):
    """Store medication information for dynamic lookup"""
    __tablename__ = "medicines"

    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(200), unique=True, index=True, nullable=False)
    brand_names = Column(JSON, nullable=True)  # List of strings
    
    # Multilingual fields (stored as JSON dicts: {"en": "...", "hi": "..."})
    uses = Column(JSON, nullable=True)
    dosage = Column(JSON, nullable=True)
